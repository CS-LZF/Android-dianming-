...
import table.Activity.editschedule.SetActivity;
import table.Activity.temp.DataBase;
import table.Activity.temp.MyApplication;
import table.Activity.temp.MyDialog;
import table.Activity.temp.ShareMethod;
...
public class KcbActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		db=new DataBase(KcbActivity.this);
...
//			finish();
	...
	//	setButton = (TextView)findViewById(R.id.setButton);
		...
		//用适配器为各选项卡添加所要显示的内容
		for(int i=0;i<7;i++){
			cursor[i]=KcbActivity.db.select(i);
		...
		exitButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				final AlertDialog.Builder builder = new AlertDialog.Builder(KcbActivity.this);
				finish();
			}
		});
...
//			@Override
//			public void onClick(View v) {
//				Intent intent = new Intent(KcbActivity.this, SetActivity.class);
					...
					if((tv.getText()).toString().equals("")){
						...
						builder.setItems(R.array.edit_options1, new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int which) {
								...
								if(which == 0){
									new MyDialog(KcbActivity.this).add(currentDay,n);
								}
							}
						});
						builder.create().show();
					...
					//否则启动修改对话框，或直接删除数据
						...
						builder.setItems(R.array.edit_options2, new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int which) {
								//如果单击了该列表项，则跳转到编辑课程信息的界面
								if(which == 0){
									new MyDialog(KcbActivity.this).modify(currentDay,n);
								}
								if(which == 1){
									...
									switch(n2){
										case 0:
											for(int m=0;m<n1;m++){
												KcbActivity.db.deleteData(currentDay,n+m+1);
											}
											break;

										case 1:
											KcbActivity.db.deleteData(currentDay,n);
											for(int m=1;m<n1;m++){
												KcbActivity.db.deleteData(currentDay,n+m);
											}
											break;
										case 2:
											KcbActivity.db.deleteData(currentDay,n-1);
											KcbActivity.db.deleteData(currentDay,n);
											for(int m=2;m<n1;m++){
												KcbActivity.db.deleteData(currentDay,n+m-1);
											}
